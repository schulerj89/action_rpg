...
const manifestPath = join(scriptDir, 'manifests', 'first-town-shop-interiors.json');
...
if (!selectedAssets.length) {
  throw new Error(`No first-town shop interior Meshy asset matched --only=${[...only].join(',')}`);
}
...
for (const asset of selectedAssets) {
  console.info(`[meshy] creating shop interior preview ${asset.id}`);
  const previewTaskId = await createTask({
    mode: 'preview',
    prompt: asset.prompt,
    ai_model: manifest.defaultModel ?? 'meshy-6',
    model_type: asset.modelType ?? manifest.defaultModelType ?? 'standard',
    should_remesh: asset.shouldRemesh ?? manifest.defaultShouldRemesh ?? true,
    topology: asset.topology ?? manifest.defaultTopology ?? 'triangle',
    ...(asset.decimationMode ?? manifest.defaultDecimationMode
      ? { decimation_mode: asset.decimationMode ?? manifest.defaultDecimationMode }
...
async function createRefinedTask(asset, previewTaskId) {
  console.info(`[meshy] creating shop interior refine ${asset.id}`);
  const refineTaskId = await createTask({
    ...
    enable_pbr: asset.enablePbr ?? manifest.defaultEnablePbr ?? false,
    hd_texture: asset.hdTexture ?? manifest.defaultHdTexture ?? false,
