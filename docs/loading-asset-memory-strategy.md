...
- Keep the first-town refined GLB kit under 25 MB until streaming and texture optimization are in place.
...
Note: `town-ground-tile.glb` is generated and preloaded, but it is not placed in the scene yet. Browser QA showed the first generated version occluding the camera, so primitive terrain remains active until the ground tile is regenerated or corrected.
